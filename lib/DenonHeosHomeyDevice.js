...
module.exports = class DenonHeosHomeyDevice extends Homey.Device {
    ...
    this.driver.getDevice(this.id).then(async speaker => {
      ...
      if(!player) {
        player = players.find(player => {        
          return player.name === this.speaker.friendlyName;
        });
      }
  ...
  async onCapabilityVolumeUp( ) {
    const { pid } = this;
    const value = await this.getCapabilityValue('volume_set');
    return this.speaker.playerSetVolume({
      pid,
      level: (value + 0.01) * 100,
    });
  }
  ...
  async playHDMI1() {
    const { pid } = this;
    return this.speaker.browsePlayInput({ pid, input: "inputs/hdmi_in_1" });
  }
   
  async playHDMI2() {
    const { pid } = this;
    return this.speaker.browsePlayInput({ pid, input: "inputs/hdmi_in_2" });
  }
   
  async playHDMI3() {
    const { pid } = this;
    return this.speaker.browsePlayInput({ pid, input: "inputs/hdmi_in_3" });
  }

  async playHDMI4() {
    const { pid } = this;
    return this.speaker.browsePlayInput({ pid, input: "inputs/hdmi_in_4" });
  }

  async playHDMIARC1() {
    const { pid } = this;
    return this.speaker.browsePlayInput({ pid, input: "inputs/hdmi_arc_1" });
  }
